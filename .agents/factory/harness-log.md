...
## 2026-07-24 — hsx-zsh-completion F1: Fix GOAL encoded an unverified root-cause hypothesis as a criterion
`decision=applied commit=daa3f33 target=.agents/skills/hs-feature/SKILL.md`
- **Rationale:** the fix's shaped GOAL baked the reporter's suspected mechanism into R4 ("must not use two conflicting, one-broken spec forms"); `/hs-plan` disproved the diagnosis and R4 had to be reinterpreted mid-lifecycle. Neither hs-feature Step 4 nor ears.md warned that a fix's root cause is *unverified at shaping time*. Two coherent touches: hs-feature Step 4 now steers `kind: fix` criteria to observable broken→fixed behavior (never the suspected cause), and ears.md gains a sibling anti-pattern to the existing "don't specify the *how*" (so hs-plan/hs-review readers benefit too). Strengthens rigor, weakens no gate; generalizes to every fix. Reinforces (does not collide with) deflake-group-retry-test F1's "root cause is unverified at planning time" (2625f94); reverts nothing.

## 2026-07-31 — part-tag-to-column F1: Commit-category rule collapsed everything non-`fix` to `feature`
`decision=applied commit=20f1290 target=.agents/skills/hs-feature/SKILL.md`
- **Rationale:** hs-feature Step 7 (`{category} = fix for kind:fix, else feature`) and hs-plan Step 8 (`{category} = fix|feature`) were both narrower than AGENTS.md's open category set and the demonstrated practice — a `kind: refactor` goal would commit as `[feature]`, yet the git log shows real `[refactor]`/`[docs]` shape+plan commits. Both lines now emit `[{kind}]` (`fix`|`feature`|`refactor`), explicitly allowing a more specific AGENTS.md category (e.g. `docs`) when the GOAL is really that kind of change; the plan commit is pinned to match the shape commit's category for lifecycle consistency. One coherent fix across the two commit lines (the finding was logged twice — `hs-feature:step7` and again `hs-plan:step8`). Strengthens fidelity to AGENTS.md, weakens no gate; reverts nothing (these lines untouched before).

## 2026-07-31 — part-tag-to-column F2: Research-depth gate ignored blast radius
`decision=applied commit=1cd3aca target=.agents/skills/hs-plan/SKILL.md`
- **Rationale:** Step 3's research-skip heuristic keyed only on `appetite`/`kind`, so a `small`/`refactor` change to the coupled core (this feature touched `data/model.py`, the highest-blast-radius file, plus a subtle SQLite JSON→column rotation) would skip the fan-out and a lean plan would get the exact contracts wrong. Added a second exception (alongside deflake-group-retry-test F1's diagnostic-fixes carve-out, 2625f94): run the full fan-out whenever the change touches any file on `invariants.md`'s **High-blast-radius files** list, regardless of `appetite`/`kind`, keying off the invariant assessment Step 2 already produces. Additive, not a revert — the two exceptions now share the `skip research` human override. Recurs across features (docs-trim F2 flagged the same "gate misses signals beyond appetite/kind"), which weights it. Strengthens rigor, weakens no gate.
